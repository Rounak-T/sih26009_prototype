...
export default function DashboardPage() {
  ...
  const avgShortfall =
    forecasts.length > 0
      ? (
          forecasts.reduce((sum, f) => sum + f.shortfall_pct, 0) /
          forecasts.length
        ).toFixed(1)
      : "0";
    ...
    <div className="p-8 space-y-8 bg-gray-50 min-h-screen">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Real-time manganese prospectivity and production risk across active
          sites
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <KPICard
          ...
          value={12}
          sublabel="from prospectivity map"
          icon={MapPinned}
          accent="emerald"
        />
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">
            {forecasts.length} mines tracked
          </span>
        </div>
          ...
          <tbody>
            {forecasts.map((f) => {
              ...
              return (
                ...
                >
                  <td className="px-5 py-3.5 font-medium text-blue-600 hover:underline">
                    <Link href={`/mines/${f.mine_id}`}>
                      {mine?.name ?? f.mine_id}
                    </Link>
                  </td>
