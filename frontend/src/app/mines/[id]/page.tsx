...
export default function MineDetailPage() {
  ...
  useEffect(() => {
    ...
    getProduction(id).then(setProduction);
    getRisks(id).then((r) => setRisk(r[0] ?? null));
    getRecommendations(id).then(setRecommendations);
  }, [id]);

  if (!mine)
    return <div className="p-8 text-gray-500">Loading mine details...</div>;
  ...
  return (
    <div className="p-8 space-y-6 bg-gray-50 min-h-screen">
      <div className="flex items-start justify-between">
        ...
        {forecast && (
          <span
            className={`px-3 py-1.5 rounded-full text-sm font-semibold ${riskColor}`}
          >
      ...
      {forecast && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <h2 className="font-semibold text-slate-900 mb-2">
            Next Period Forecast — {forecast.period}
          </h2>
          <div className="grid grid-cols-3 gap-4 mt-3">
            <div>
              <p className="text-xs text-gray-500">Planned</p>
              <p className="text-xl font-bold text-slate-900">
                {forecast.planned_tonnes.toLocaleString()} t
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Forecast</p>
              <p className="text-xl font-bold text-slate-900">
                {forecast.forecast_tonnes.toLocaleString()} t
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Shortfall</p>
              <p className="text-xl font-bold text-red-600">
                {forecast.shortfall_pct}%
              </p>
            </div>
          </div>
        </div>
